...
void RoleBehavior::OnInit() {
    ...
    controller_ = std::make_unique<PlayerController>(Parent(), hair_, head_, body_, feet_, 200, 1);
}
...
void RoleBehavior::initRoleBody() {
    ...
    auto tilesheet = engine::TileSheetFactory::Find("role");
    auto sprite = hair_->GetComponent<engine::SpriteComponent>();
    sprite->image = tilesheet->Get(1, 1);

    sprite = body_->GetComponent<engine::SpriteComponent>();
