...
void CreateEntities(ecs::Commands& cmd, ecs::Resources resources) {
    ...
    ecs::Entity root = cmd.SpawnAndReturn<Shape>(Shape::CreateRect(center, math::Vector2(200, 200), Color::Red));
    HierarchyBuilder(cmd, root).SetChilds(
        {
            cmd.SpawnAndReturn<Shape>(Shape::CreateRect(center, math::Vector2(150, 150), Color::Green))
        }
    );
