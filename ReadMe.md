...
`GameObject`是仿照`Unity`的同名类制作的。其属于ECS中的`Entity`部分。可以在上面挂载很多Component。

`Component`顾名思义，就是可以被挂载的组件。

使用`GameObject::AddComponent<T>(component)`可以增加组件，这时会调用组件的`Init()`函数。

使用`GameObject::RemoveComponent<T>(component)`可以删除组件。这时**不会调用组件的`Destroy()`函数**，用户必须手动调用。
