...
public:
    ...
    void Trace(std::string_view funcName, std::string_view filename,
               unsigned int line, Args &&...args) {
        log(Level::Trace, funcName, filename, line,
            std::forward<Args>(args)...);
    }

    template <typename... Args>
    void Debug(std::string_view funcName, std::string_view filename,
               unsigned int line, Args &&...args) {
        log(Level::Debug, funcName, filename, line,
            std::forward<Args>(args)...);
    }

    template <typename... Args>
    void Info(std::string_view funcName, std::string_view filename,
              unsigned int line, Args &&...args) {
        log(Level::Info, funcName, filename, line, std::forward<Args>(args)...);
    }

    template <typename... Args>
    void Warning(std::string_view funcName, std::string_view filename,
                 unsigned int line, Args &&...args) {
        log(Level::Warning, funcName, filename, line,
            std::forward<Args>(args)...);
    }

    template <typename... Args>
    void Error(std::string_view funcName, std::string_view filename,
               unsigned int line, Args &&...args) {
        log(Level::Error, funcName, filename, line,
            std::forward<Args>(args)...);
    }

    template <typename... Args>
    void FatalError(std::string_view funcName, std::string_view filename,
                    unsigned int line, Args &&...args) {
        log(Level::FatalError, funcName, filename, line,
            std::forward<Args>(args)...);
    }
...
private:
    ...
    template <typename... Args>
    void log(Level level, std::string_view funcName, std::string_view filename,
             unsigned int line, Args &&...args) {
        if (level <= level_) {
            printf("[%s][%s][%s][%d]", Level2Str(level).data(), filename.data(),
                   funcName.data(), line);
...
public:
    ...
    static Logger &CreateFromFile(const std::string &name,
                                  const std::string &filename,
                                  bool append = false) {
        auto &instance = Instance();
        instance.files_.emplace_back(filename,
                                     append ? std::ios::app : std::ios::trunc);
        return instance.loggers_.emplace(name, Logger(instance.files_.back()))
            .first->second;
    }
